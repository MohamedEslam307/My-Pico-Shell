...
int main()
{
	...
	if (parsed_input_string != NULL) {
		...
		command_line_node_ptr->num_of_redirections=node_ptr->num_of_redirections;
		num_of_redirections=node_ptr->num_of_redirections;

		for (int i = 0; i < node_ptr->num_of_arguments; i++) {
...
char my_echo(Node * cmd_line_data)
{
	...
	int echo_arguments =
	    (cmd_line_data->num_of_arguments -
	     (cmd_line_data->num_of_redirections * 2));
	/*printf("num of arguments = %d \n",
	   cmd_line_data->num_of_arguments);
	   printf("num of echo arguments = %d \n", echo_arguments);
	 */
	...
	if (1 == echo_arguments) {
	    ...
	    for (arg_index = 1; arg_index < echo_arguments; arg_index++) {
...
char my_cd(char **parsed_input_string, char arguments_size)
{
	...
	for (int i = 0; i < arguments_size; i++) {
	    printf("%s ", parsed_input_string[i]);
	}
	printf("\n");
	if (2 == arguments_size) {
